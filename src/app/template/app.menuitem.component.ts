...
@Component({
    ...
    template: `
		<ng-container>
			...
			<ul *ngIf="((item.items && root) || (item.items && active)) && item.visible !== false" class="menu-border-top"
				[@children]="(root ? 'visible' : active ? 'visibleAnimated' : 'hiddenAnimated')">
				<ng-template ngFor let-child let-i="index" [ngForOf]="item.items">
					<li app-menuitem [item]="child" [index]="i" [parentKey]="key" [class]="child.badgeClass" class="bg-white"></li>
				</ng-template>
			</ul>
		</ng-container>`,
    host: {
